...
export class AppSettingsService {
  ...
  async update(dto: { lead_magnets?: string[]; lead_sources?: string[]; custom_gateways?: string[]; callBookingUrl?: string; exchangeRates?: Record<string, number> }): Promise<AppSettingsDocument> {
    let doc = await this.model.findOne()
    if (!doc) doc = await this.model.create({})
    ...
    if (dto.callBookingUrl  !== undefined) doc.callBookingUrl  = dto.callBookingUrl
    if (dto.exchangeRates   !== undefined) doc.exchangeRates   = new Map(Object.entries(dto.exchangeRates))
    return doc.save()
  }
}
