...
export class ContentService {
  ...
  async quickStructure(rawIdea: string): Promise<{
    ...
    if (!apiKey) throw new BadRequestException('ANTHROPIC_API_KEY non configuré dans .env')

    const prompt = `Tu es un expert en création de contenu pour le e-commerce en Afrique francophone.
...
Choisis le format le plus adapté à l'idée.`
    ...
    const resp = await axios.post<{ content: Array<{ text: string }> }>(
    ...
    const jsonStr = raw.startsWith('```') ? raw.replace(/^```(?:json)?\s*/, '').replace(/\s*```$/, '') : raw

    try {
      return JSON.parse(jsonStr)
    } catch {
      throw new BadRequestException('Erreur parsing réponse Claude — réessayez.')
    }
  }
  ...
  async analyzeProject(projectId: string): Promise<VideoProjectDocument> {
    ...
    const prompt = `Tu es un expert en création de contenu, spécialisé dans le e-commerce et l'entrepreneuriat en Afrique francophone.
...
Format : ${fmt.name} — ${isShort ? 'Format COURT (60-90 secondes)' : 'Format LONG (5-15 minutes)'}
...
- Commencer DIRECTEMENT par le script`
    ...
    const script = resp.data.content[0].text.trim()
  ...
  async generateDailySuggestions(userId: string): Promise<ContentSuggestionDocument[]> {
    ...
    const prompt = `Tu es un expert en stratégie de contenu pour le e-commerce en Afrique francophone.
