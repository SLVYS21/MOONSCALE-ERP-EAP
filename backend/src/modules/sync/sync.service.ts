...
export class SyncService {
  ...
  async backfillProofImages(): Promise<{ updated: number; skipped: number; noPayment: number }> {
    ...
    for (const resp of responses) {
      ...
      // Télécharger depuis Tally et re-uploader sur Cloudinary
      // (URLs Tally ont des JWT qui expirent — Cloudinary = URLs permanentes accessibles depuis le serveur)
      const cloudinaryUrls: string[] = []
      for (const tallyUrl of urls) {
        try {
          const resp = await axios.get<ArrayBuffer>(tallyUrl, {
            responseType: 'arraybuffer',
            timeout: 30_000,
            headers: { 'User-Agent': 'Mozilla/5.0' },
          })
          const buffer = Buffer.from(resp.data)
          const cloudUrl = await this.cloudinaryService.upload(buffer, 'payment-proofs')
          cloudinaryUrls.push(cloudUrl)
        } catch (err: unknown) {
          this.logger.warn(`Backfill: upload Cloudinary échoué pour ${tallyUrl.slice(0, 60)}... — ${(err as Error).message}`)
        }
      }

      if (cloudinaryUrls.length === 0) { skipped++; continue }

      payment.proofImages = cloudinaryUrls
      if (tallySubmissionId && !payment.tallySubmissionId) {
        payment.tallySubmissionId = tallySubmissionId
      }
      await payment.save()
      updated++
      this.logger.log(`Backfill: ${payment.studentEmail} — ${cloudinaryUrls.length} preuve(s) migrée(s) sur Cloudinary`)
    }
